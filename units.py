...
def load_data(training_file, validation_file, testing_file):
    ...
    print('dddd',train.shape,validate.shape)
...
def pad_matrix_new(seq_values):
    batch_values_codes = np.zeros((1,24, 2), dtype=np.float32) 
    # for pid, subseq in enumerate(seq_values):
    #     # print('pid,subseq',pid, subseq)
    #     for tid, code in enumerate(subseq):
    #         batch_values_codes[pid, tid] = code

    for bid, seq in enumerate(seq_values):
        #print('bid,seq',bid,seq)
        for pid, subseq in enumerate(seq):
            #print('pid,subseq',pid, subseq)
            for tid, code in enumerate(subseq):
                  batch_values_codes[bid, pid, tid] = code
    
    return batch_values_codes
...
def calculate_cost_tran(model, data, options, max_len, loss_function=F.cross_entropy):
    ...
    for index in range(n_batches):
        ...
        batch_values_codes = data[3][batch_size * index: batch_size * (index + 1)]
        batch_diagnosis_codes, batch_time_step, batch_values_codes = adjust_input(batch_diagnosis_codes, batch_time_step, max_len, options['n_diagnosis_codes'], batch_values_codes)
        batch_labels = data[1][batch_size * index: batch_size * (index + 1)]
        lengths = np.array([len(seq) for seq in batch_diagnosis_codes])
        maxlen = np.max(lengths)
        logit, labels, self_attention = model(batch_diagnosis_codes, batch_time_step, batch_labels, options, maxlen, batch_values_codes)
        loss = loss_function(logit, labels)
        # print(loss,logit,labels)
    ...
    # print(data[3][56])
    model.train()
    # print(cost_sum,n_batches)
    return cost_sum / n_batches


def adjust_input(batch_diagnosis_codes, batch_time_step, max_len, n_diagnosis_codes, batch_values):
    batch_time_step = copy.deepcopy(batch_time_step)
    batch_diagnosis_codes = copy.deepcopy(batch_diagnosis_codes)
    batch_values = copy.deepcopy(batch_values)
    print('len',len(batch_diagnosis_codes))
    for ind in range(len(batch_diagnosis_codes)):
        if len(batch_diagnosis_codes[ind]) > max_len:
            ...
            batch_values[ind] = batch_values[ind][-(max_len):]
        #batch_time_step[ind].append(0)
        #batch_diagnosis_codes[ind].append([n_diagnosis_codes - 1])
        #batch_values[ind].append([0.])
    return batch_diagnosis_codes, batch_time_step, batch_values
